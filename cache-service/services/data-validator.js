...
export class DataValidator {
  ...
  validate(newData, previousData, dataType = 'default') {
    ...
    switch (dataType) {
      ...
      case 'protocol-info':
        return this.validateProtocolInfo(newData, previousData);
      
      case 'protocol-revenue':
        return this.validateGeneric(newData, previousData);
      
      case 'all-protocols':
        return this.validateProtocolList(newData, previousData);
      
      case 'volume-data':
        return this.validateVolumeData(newData, previousData);
  ...
  /**
   * Validate protocol info data
   */
  validateProtocolInfo(newData, previousData) {
    // Protocol info should have basic fields
    if (!newData.name && !newData.slug && !newData.description) {
      return {
        isValid: false,
        reason: 'Protocol info missing basic fields',
        useStale: true
      };
    }
    ...
    return {
      isValid: true,
      reason: 'Protocol info passed validation'
    };
  }

  /**
   * Validate protocol list data (all-protocols)
   */
  validateProtocolList(newData, previousData) {
    // Should be an array with protocols
    if (!Array.isArray(newData) && !newData.protocols) {
      return {
        isValid: false,
        reason: 'Protocol list is not an array or missing protocols field',
        useStale: true
      };
    }

    const protocols = Array.isArray(newData) ? newData : newData.protocols;
    ...
    // If we had protocols before and now we have significantly fewer, that's suspicious
    if (previousData) {
      const prevProtocols = Array.isArray(previousData) ? previousData : previousData.protocols;
      if (prevProtocols && prevProtocols.length > 100 && protocols.length < prevProtocols.length * 0.5) {
        return {
          isValid: false,
          reason: `Protocol list dropped from ${prevProtocols.length} to ${protocols.length} (suspicious)`,
          useStale: true
        };
      }
    }
    ...
    return {
      isValid: true,
      reason: 'Protocol list passed validation'
    };
  }
   ...
   */
  validateVolumeData(newData, previousData) {
    // Volume can legitimately be 0
    const volume = newData.volume_24h || newData.volume || newData.data;

    // But if it's null or explicitly unavailable, reject
    if (volume === null || newData._unavailable) {
      return {
        isValid: false,
        reason: 'Volume is null or unavailable',
        useStale: true
      };
    }
    ...
    return {
      isValid: true,
      reason: 'Volume data passed validation'
    };
  }
